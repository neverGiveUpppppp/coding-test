...
brainstorming
    ...
    3)즐찾max : FETCH +@ 서브쿼리(?)
        각 FOOD_TYPE 별로 WHERE문 주고 UNION하기
...
-- -- 여기서 각각 푸드타입별 FAVORITES이 높은 거 하나씩 추출 필요

-- 2.각 푸드타입별 FAVORITES이 높은 거 하나씩 추출 : but, REST_ID REST_NAME 필요
...
-- -- 조회결과가 그냥 하나 GROUP 여러 지정한 거랑 똑같이 한식한식한식 나옴

-- 둘 합치기 시도2 
-- SELECT A.FOOD_TYPE, B.REST_ID, B.REST_NAME, MAX(A.FAVORITES) AS FAVORITES
-- FROM REST_INFO A
--     JOIN (
--         SELECT FOOD_TYPE, REST_ID, REST_NAME, FAVORITES
--         FROM REST_INFO
...
-- -- 서브쿼리 역방향으로 해보자. 어차피 GROUP 안에 없는거라 조회 불가ㅜㅠ
-- -- 서브쿼리 없이도 문장 하나로 끝내야할 것 같은데?

-- SELECT DISTINCT(FOOD_TYPE) FOOD_TYPE, REST_ID, REST_NAME, MAX(FAVORITES)
...
-- FROM REST_INFO
-- WHERE FOOD_TYPE = '중식'
-- 734, 20 두개 찍힘 -> 프로그래머시 UNION ALL 작동
...
--     MAX(FAVORITES) OVER (PARTITION BY REST_NAME )  AS FAVORITES
-- FROM REST_INFO
-- ORDER BY FOOD_TYPE DESC
...
-- 정답1 : WHERE + IN + SUBQUERY + GROUP BY
SELECT FOOD_TYPE, REST_ID, REST_NAME, FAVORITES
FROM REST_INFO
...
-- 정답2 : JOIN ON 2개 + SUBQUERY + GROUP BY
...
FROM REST_INFO A
    ...
    ON A.FOOD_TYPE = B.FOOD_TYPE  -- JOIN ON 2개
    AND A.FAVORITES = B.FAVORITES 
ORDER BY A.FOOD_TYPE DESC
-- JOIN ON을 2개하는거나 JOIN ON 1개에 WHERE로 하는거나 거의 비슷하나 JOIN을 연속으로 할 경우 조회 데이터가 달라질 수 있음. ON을 두개 하는건 GROUP BY나 ODRDER BY 2개 거는 것과 같음

-- 정답3 : SUBQUERY + ROW_NUBER() + OVER절 
SELECT FOOD_TYPE, REST_ID, REST_NAME, FAVORITES
FROM (
        SELECT  A.*,
