...
조건
    ...
    2)해당 기간 동안의 월별 자동차 ID 별 총 대여 횟수
    3)월을 기준으로 오름차순 정렬, 월이 같다면 자동차 ID를 기준으로 내림차순 정렬
...
---- 1. 2022년 8월부터 2022년 10월까지(시작일 기준)
-- SELECT HISTORY_ID, CAR_ID, TO_CHAR(START_DATE, 'YYYY-MM-DD')
...
-- -- 대여5회 COUNT개수 검증
-- SELECT HISTORY_ID, CAR_ID, COUNT(CAR_ID) -- CAR_ID 이외에 다른 컬럼 조회해보면 COUNT(CAR_ID)가 맞는 개수인지 확인 가능
-- FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
-- WHERE TO_CHAR(START_DATE, 'YYYY-MM-DD') BETWEEN '2022-08-01' AND '2022-10-31' 
-- GROUP BY CAR_ID, HISTORY_ID
-- ORDER BY CAR_ID
...
---- 3.MONTH 및 RECORED 결과 구현
-- SELECT EXTRACT(MONTH FROM A.START_DATE) AS MONTH, B.CAR_ID, B.RECORDS
...
-- HAVING COUNT(CAR_ID) >= 5 
-- ORDER BY /*MONTH ASC,*/ CAR_ID DESC 
--     -- START_DATE만 추가했더니 결과 안나옴. 이놈이 원인

-- SELECT START_DATE, CAR_ID, COUNT(CAR_ID) AS RECORDS
-- FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
-- WHERE TO_CHAR(START_DATE, 'YYYY-MM-DD') BETWEEN '2022-08-01' AND '2022-10-31' 
-- GROUP BY CAR_ID, START_DATE 
-- HAVING COUNT(CAR_ID) >= 5 
-- ORDER BY /*MONTH ASC,*/ CAR_ID DESC 
--     -- START_DATE를 SELECT에 추가해도 조회 안됨. HAVING 조건 때문
...
-- ORDER BY /*MONTH ASC,*/ CAR_ID DESC 
--     -- HAVING COUNT(CAR_ID) >= 5 조건을 만조갛려면, group by에 card_id 하나만 필요. start_date 붙이면 안나옴.


-- SELECT EXTRACT(MONTH FROM A.START_DATE) AS MONTH, A.CAR_ID, B.RECORDS
-- FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY A
--     JOIN (
--         SELECT CAR_ID, COUNT(CAR_ID) AS RECORDS
--         FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY
...
--     ON A.CAR_ID = B.CAR_ID
-- ORDER BY MONTH ASC, CAR_ID DESC
--     -- INNER JOIN하면 GROUP BY했던게 날라감. 정답X
...
-- WHERE TO_CHAR(START_DATE, 'YYYY-MM-DD') BETWEEN '2022-08-01' AND '2022-10-31' 
GROUP BY CAR_ID, START_DATE
HAVING TO_CHAR(START_DATE, 'YYYY-MM-DD') BETWEEN '2022-08-01' AND '2022-10-31' 
ORDER BY MONTH ASC, CAR_ID DESC
    -- start_date까지 나오긴 하나 전부 group by로 인해 날짜별로 다 세분화되서 정답X
